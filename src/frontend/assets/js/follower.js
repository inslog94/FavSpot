export function createFollower(requestUserPk, followingList) {
    ...
    .then((data) => {
      ...
      if ((requestUserPk === parseInt(pk)) | (pk == 'me')) {
        ...
        button.addEventListener('click', () => {
          fetch(`https://favspot.site:8443/user/follow/${pk}/`, {
            method: 'DELETE',
            credentials: 'include',
            headers: {
  ...
  const url =
    pk === 'me'
      ? `https://favspot.site:8443/user/follower/`
      : `https://favspot.site:8443/user/${pk}/follower/`;
  // 팔로워 목록
  fetch(url, {
    method: 'GET',
    credentials: 'include',
