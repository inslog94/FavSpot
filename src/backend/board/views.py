    ...
    def get(self, request, format=None):
        ...
        if not request.user.is_authenticated:
            ...
            if search_field == 'all':
                ...
                for term in search_term:
                    queries |= Q(title__icontains=term) | Q(tags__content__icontains=term) | Q(pin__title__icontains=term)
                queryset = queryset.annotate(pin_count=Count('pin')).filter(pin_count__gt=0).filter(queries).distinct()
        ...
        # 로그인된 사용자의 경우
            ...
            if search_field == 'all':
                ...
                queries_public = Q()
                for term in search_term:
                    queries_user |= Q(title__icontains=term) | Q(tags__content__icontains=term) | Q(pin__title__icontains=term)
                    queries_public |= Q(title__icontains=term) | Q(tags__content__icontains=term) | Q(pin__title__icontains=term)
                user_boards_filtered = user_boards.annotate(pin_count=Count('pin')).filter(pin_count__gt=0).filter(queries_user).distinct()
                public_boards_filtered_except_user_ones = public_boards_except_user_ones.annotate(pin_count=Count('pin')).filter(pin_count__gt=0).filter(queries_public).distinct()
